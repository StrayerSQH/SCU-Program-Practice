...
int ChapterOne::Program(sf::RenderWindow& window)
{
	...
	while (window.isOpen())
	{
		...
		if (!guideButtonOneisPrint && !guideButtonSecondisPrint && !guideButtonThirdisPrint)
		{
			guideButtonOneisPrint = guideButtonOne.DrawButton(window);
			std::this_thread::sleep_for(std::chrono::milliseconds(2000));
		}
		else if (guideButtonOneisPrint && !guideButtonSecondisPrint && !guideButtonThirdisPrint)
		{
